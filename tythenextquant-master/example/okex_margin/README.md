...
本文主要介绍如何通过框架SDK开发 [OKx 杠杆](https://www.okx.me/) 交易所的交易系统。
...
- 注册 [OKx 杠杆](https://www.okx.me/) 的账户，并且创建 `ACCESS KEY` 和 `SECRET KEY`，AK有操作委托单权限；
...
> 注意：这里我们在配置文件里需要配置 `passphrase` 参数，即 OKx 的 API KEY 的密码，同时在初始化 `Trade` 模块的时候传入此参数。
...
- ACCOUNTS 指定需要使用的交易账户，注意platform是 `okex_margin`，并且需要配置 `passphrase` 参数，即 OKx 的 API KEY 的密码；
- strategy 策略的名称；
- symbol 策略运行的交易对；
