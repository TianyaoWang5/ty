...
本文主要介绍如何通过框架SDK开发 [OKx Future](https://www.okx.me/future/swap) 交易所的交易系统。
...
- 注册 [OKx Future (交割合约)](https://www.okx.me/future/swap) 的账户，并且创建 `ACCESS KEY` 和 `SECRET KEY`，AK有操作委托单权限；
...
- ACCOUNTS 指定需要使用的交易账户，注意platform是 `okex_future`，并且需要配置 `passphrase` 参数，即 OKx 的 API KEY 的密码；
- strategy 策略的名称；
