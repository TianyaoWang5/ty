...
class Tymain:
    ...
    async def gotasks(self):
        tasks = [asyncio.create_task(self.get_orderbook_update()), asyncio.create_task(self.check_asset_update())]
    ...
    async def mainloop(self,*args, **kwargs):
        ...
        try:
            ...
            # 查询当前挂单
            openOrders, error = await self.ba_rest_api.exchangeIO("GET", "/api/v3/openOrders", param)
            self.TY.Log(openOrders,error)
            return
