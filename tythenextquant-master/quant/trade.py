...
class Trade:
    ...
    Attributes:
        ...
        platform: Exchange platform name. e.g. `binance` / `okx` / `bitmex`.
        ...
        passphrase: API KEY Passphrase. (Only for `OKx`)
    ...
    def __init__(self,ispublic_to_mq=False,islog=False, strategy=None, platform=None, symbol=None, host=None, wss=None, account=None, access_key=None,
        ...
        if platform == const.OKX:
            from quant.platform.okx import OKExTrade as T
    ...
    async def create_order(self, action, price, quantity, order_type=ORDER_TYPE_LIMIT, resptype = "FULL",**kwargs):
        """ Create an order.
        ...
        """
        order_no, error = await self._t.create_order(action, price, quantity, order_type, resptype, **kwargs)
